...
lines_i=($(awk '{ print $1/1000+1 }' *.RAWobserved))
lines_j=($(awk '{ print $2/1000+1 }' *.RAWobserved))

IFS=_ read var1 var2 <<< "$x"
echo $var1
echo $var2

for NUMi in ${lines_i[@])}; do
sed "${NUMi}q;d" ${var1}_1kb.KRnorm >>norm.i
done

for NUMj in ${lines_j[@])}; do
sed "${NUMj}q;d" ${var2}_1kb.KRnorm >>norm.j
done

wc -l norm.i norm.j *.RAWobserved
cat *.RAWobserved norm.i norm.j > tmp
awk '{ print $1, "\t", $2, "\t", $3/($4*$5)}' tmp >${x}_1kb.NORMobserved
