...
log_con <- file("godmc_phase2_analysis/09_garfield/CisTransThresAnno.log")


uk10k<-NULL
for(chr in 1:22){
	tmp<-fread(paste("godmc_phase2_analysis/09_garfield/maftssd/chr", chr, sep = ""))
...
for(i in 1:962){
	...
	load(paste0("godmc_phase2_analysis/16_jan2018/16_cleaned_", i, ".rdata"))
...
cat(paste("Number of tran variants loaded from QTL meta-analysis :", nrow(a.minP.trans)), file = log_con)
...
for(chr in 1:22){
	...
	write.table(a.minP.comb[,c(1,4)],paste("godmc_phase2_analysis/09_garfield/annotationCisTrans/chr", chr, sep = ""), col.names = FALSE, row.names = FALSE, sep = " ", quote = FALSE)
}
...
write.table(linkFile, "godmc_phase2_analysis/09_garfield/annotationCisTrans/link_file.txt", row.names = FALSE, sep = " ", quote = FALSE)
