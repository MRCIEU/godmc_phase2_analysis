...
path="/panfs/panasas01/shared-godmc/godmc_phase2_analysis/scratch/control"
...
#a<-read.table(paste(path,"/results/16/control/16_1.txt.gz",sep=""),he=T)
#a2<-a[which(a$cis==F & a$Pvalue<5e-8),]

a<-read.table("/panfs/panasas01/sscm/epzjlm/repo/godmc_phase2_analysis/03_clumping_16/clumped.txt",he=T)
...
l<-list.files(path=path,pattern=".control.merged")
#l2<-list.files(paste(cohort_dir,"/",l[1],"/results/16/",sep=""))
study<-gsub("_16.control.merged","",l)
...
r<-read.table(paste(path,"/",l[i],sep=""),he=T)
m<-which(r$MARKERNAME%in%as.character(a[,1]))
...
save.image("/panfs/panasas01/sscm/epzjlm/repo/godmc_phase2_analysis/01_meta_analysis_16/mstatscontrol.RData")

res2<-res[which(res$P<1e-5),]
