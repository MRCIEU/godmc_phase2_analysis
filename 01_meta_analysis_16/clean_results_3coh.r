...
setwd("/mnt/storage/private/mrcieu/research/GODMC_Analysis/godmc_phase2_analysis/01_meta_analysis_16")
...
out <- paste0("../results_3coh/16_3cohorts/16_cleaned_", i, ".rdata")
...
message(i)
a <- fread(paste0("zcat ../results_3coh/16_3cohorts/16_", i, ".txt.gz"))
...
ss<-(5000/27750)*max(a$TotalSampleSize)
nstudies <- nchar(a$Direction[1])
nst <- nstudies - str_count(a$Direction, "\\?")
rem <- a$TotalSampleSize < ss
cleaning_message(sum(rem), " associations with fewer than", round(ss,2), " samples removed")
