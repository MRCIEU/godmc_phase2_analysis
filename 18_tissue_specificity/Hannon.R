...
m<-match(cl3$id,h2$id)
h3<-h2[m,]

out<-calcu_cor_true(b1=cl3$Effect,se1=cl3$StdErr,b2=h3$BETA2,se2=h3$SE,theta=0)
cor.out<-unlist(out)

cor.out<-data.frame(cor.out,category="cis")
save(cor.out,file="rb.filtered_primary_hannon_cis.Robj")
