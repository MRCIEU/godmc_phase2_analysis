...
temp <- inner_join(
	subset(clumped, select=c(snp, cpg, Allele1, Allele2, Effect, StdErr, StdErrARE, StdErrMRE, pval, TotalSampleSize, HetISq, cis)),
...
ggplot(temp, aes(x=Effect, y=beta1)) +
geom_point(aes(colour=gs.sig)) +
facet_grid(. ~ cist) +
labs(x="GoDMC effect size", y="GS effect size")
```
...
r <- preplication(temp$Effect, temp$StdErrMRE, temp$se, 0.05/nrow(clumped))
# sum(temp$gs.sig)
...
```{r, echo=FALSE}
r <- temp %$% preplication(Effect, StdErrMRE, se, 0.05/nrow(clumped))[[3]]
...
p <- sum(r$cgid %in% temp2$cpg)
```

* There are `r nrow(r)` CpGs with significant h2 estimates in van Dongen. 
* `r p` were present in the GS replication dataset

What proportion of the h2 is explained by mQTLs?

```{r, echo=FALSE}
vd <- inner_join(temp2, r, by=c("cpg"="cgid"))
vd$ord <- rank(vd$h2_twinAE)

vd2 <- gather(subset(vd, select=c(r2, h2_twinAE, ord)),key, value, -ord)
...
```{r, }
ggplot(vd2, aes(x=ord, y=value)) +
geom_point(aes(colour=key)) +
geom_smooth(data=subset(vd2, key=="r2"))
```
