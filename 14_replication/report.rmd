...
gs1<-subset(gs,info>0.8 & freq1>0.01) #172448
w<-which(gs1$cpg%in%p$x)
gs1<-gs1[-w,] #172147
fail<-nrow(gs)-nrow(gs1) #1498
...
temp <- inner_join(
	subset(clumped, select=c(snp, cpg, Allele1, Allele2, Freq1, Effect, StdErr, StdErrARE, StdErrMRE, pval, TotalSampleSize, HetISq, cis, code)),
...
labs(x="GoDMC effect size", y="Generation Scotland effect size", colour="Significant\nreplication") +
...
labs(x="CpG variance explained", fill="mQTLs")
...
labs(x="Number of mQTLs per CpG", y="Heritability estimate", fill="Heritability\nstudy design")
