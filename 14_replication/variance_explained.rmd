...
replication_ <- group_by(replication, cpg) %>%
	summarise(r2=sum(r2))
replication2$what <- "hsqtot"
...
bins2 <- cut(replication_mean_sd$sdcpg, breaks, include.lowest = T, right=FALSE, labels=labels)

replication_mean_sd<-data.frame(replication_mean_sd,mean.bins=bins,sd.bins=bins2)

ggplot(replication_mean_sd,aes(x=replication2$mean.bins,y=r2)) +
geom_boxplot() +
labs(x="Weighted mean DNA methylation level",y="variance explained (r2)")
ggsave("images/meanbeta_r2.pdf")

ggplot(replication_mean_sd,aes(x=replication_mean_sd$mean.bins,y=r2)) +
geom_boxplot() +
...
What proportion of h2 is explained? To summarise this set probes with h2==0 to h2==1e-10 because this gives division errors.
...
p1<-ggplot(vd, aes(x = prop)) +  
geom_histogram()
ggsave(p1,file="images/prop_vandongen.pdf")
```
...
p1<-ggplot(vd, aes(x = prop)) +  
geom_histogram()
ggsave(p1,file="images/prop_vandongen_all.pdf")

p1<-ggplot(vd[-w0,], aes(x = prop)) +  
geom_histogram()
ggsave(p1,file="images/prop_vandongen_no_h2eq0.pdf")
...
p1<-ggplot(mc, aes(x = prop)) +  
geom_histogram()
ggsave(p1,file="images/prop_mcrae.pdf")

p1<-ggplot(mc2, aes(x = prop)) +  
geom_histogram()
ggsave(p1,file="images/prop_mcrae_no_h2eq0.pdf")
...
tab2l %>% group_by(key) %>% summarise(cor=cor(mqtls, value)^2)
tab2l %>% group_by(key) %>% summarise(cor=cor(mqtls, value,use="complete.obs"))
tab2l %>% filter(key == "Twin") %>% lm(r2 ~ value, data=.) %>% summary %>% coefficients
tab2l %>% filter(key == "Family") %>% lm(r2 ~ value, data=.) %>% summary %>% coefficients
...
w1<-which(tab3$genetic_r2_mcrae=="Inf")
w2<-which(tab3$genetic_r2_vandongen=="Inf")
tab3$genetic_r2_vandongen[w2]<-NA
tab3$genetic_r2_mcrae[w1]<-NA
