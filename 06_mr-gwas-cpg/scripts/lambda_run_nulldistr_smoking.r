...
smok_cpgs<-unique(c(smok$Name,chr6_cpgs))


tr <- data.frame(id=1:length(traits), trait=traits)
lam <- expand.grid(smok=c(TRUE, FALSE), id=1:length(traits), ncpg=NA, nsnp=NA, minp=NA, nsig=NA, lambda=NA, proppos=NA, pdir=NA)
...
lama <- subset(lam, smok)
lamb <- subset(lam , !smok)
