...
res_all$userSet[w]<-paste0("ambivalent (", l[1]," regions)")
...
p1<-ggplot(res_all, aes(fill=userSet, y=-log10(res_all$pvalue), x=description)) + 
...
p2<-ggplot(res_all, aes(color=userSet, y=logOddsRatio, x=description)) +
 #scale_fill_brewer(type="qual") +
    #geom_bar(position="dodge", stat="identity") +
    geom_point(position=position_dodge(.9)) +
...
load("../results/enrichments/lola_gene_mqtlcpg_cis.rdata")
res_all1<-res_all
res_all1$cis<-"1Mb"
