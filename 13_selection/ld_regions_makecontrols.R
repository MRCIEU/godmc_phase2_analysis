...
f.all$MAFquantile <- cut(log(f.all$MAF+0.001), breaks=c(quantile(log(f.all$MAF+0.001),probs = seq(0, 1, by = 0.20))), labels=c("0-20","20-40","40-60","60-80","80-100"), include.lowest=TRUE)
table(f.all$MAFquantile)

f.all$nproxiesquantile <- cut(log(f.all$nproxies+0.001), breaks=c(quantile(log(f.all$nproxies+0.001),probs = seq(0, 1, by = 0.25))), labels=c("0-25","25-50","50-75","75-100"), include.lowest=TRUE)
table(f.all$nproxiesquantile)

f.all$tssdistquantile <- cut(log(abs(f.all$tssdist)+0.001), breaks=c(quantile(log(abs(f.all$tssdist)+0.001),probs = seq(0, 1, by = 0.20))), labels=c("0-20","20-40","40-60","60-80","80-100"), include.lowest=TRUE)
table(f.all$tssdistquantile)

f.all$closest450kdistancequantile <- cut(log(abs(f.all$closest450kdistance)+0.001), breaks=c(quantile(log(abs(f.all$closest450kdistance)+0.001),probs = seq(0, 1, by = 0.20))), labels=c("0-20","20-40","40-60","60-80","80-100"), include.lowest=TRUE)
...
p1 <- ggplot(f.all, aes(MAF, fill=mqtl_clumped)) + 
geom_density(alpha = 0.2) 
p2 <- ggplot(f.all, aes(nproxies, fill=mqtl_clumped)) + 
geom_density(alpha = 0.2) +
xlim(0,250)
p3 <- ggplot(f.all, aes(tssdist, fill=mqtl_clumped)) + 
geom_density(alpha = 0.2) +
xlim(0,400000)
p4<-ggplot(f.all,aes(closest450kdistance,fill=mqtl_clumped)) +
geom_density(alpha = 0.2) +
xlim(0,100000)
p5 <- ggplot(f.all, aes(CpG_freq, fill=mqtl_clumped)) + 
geom_density(alpha = 0.2) +
xlim(0,0.25)
p6 <- ggplot(f.all, aes(GC_freq, fill=mqtl_clumped)) + 
geom_density(alpha = 0.2)
...
p1 <- ggplot(f.all, aes(log(MAF+0.001), fill=mqtl_clumped)) + 
geom_density(alpha = 0.2) 
p2 <- ggplot(f.all, aes(log(nproxies+0.001), fill=mqtl_clumped)) + 
geom_density(alpha = 0.2) +
xlim(0,10)
p3 <- ggplot(f.all, aes(log(abs(tssdist)+0.001), fill=mqtl_clumped)) + 
geom_density(alpha = 0.2) +
xlim(0,15)
p4<-ggplot(f.all,aes(log(abs(closest450kdistance)+0.001),fill=mqtl_clumped)) +
geom_density(alpha = 0.2) +
xlim(0,15)
p5 <- ggplot(f.all, aes(CpG_freq, fill=mqtl_clumped)) + 
geom_density(alpha = 0.2) +
...
pdf("./images/nproxies.pdf",height=6,width=6)
par(mfrow=c(2,2))
...
##select groups with mqtl_clumpeds

t<-table(f.all$groups_quantiles2,f.all$mqtl_clumped)
dim(t)
#11884
m<-match(f.all$groups_quantiles2,row.names(t))
t2<-t[m,]
f.all<-data.frame(f.all,Ncontrols_6prop=t2[,1], Nmqtl_clumped_6prop=t2[,2])
...
w<-which(t[,1]-t[,3]<0)
cat(nrow(t[w,]),"controlgroups have less than 10 times mqtls snps","\n")

t$req5<-5*as.numeric(t[,2])
w<-which(t[,1]-t[,4]<0)
cat(nrow(t[w,]),"controlgroups have less than 5 times mqtls snps","\n")

w<-which(t[,1]-t[,2]<0)
cat(nrow(t[w,]),"controlgroups < mqtls snps","\n")
...
xlab("groupsizes")
ggsave(plot=p1, file="./images/groupsizesmqtl_clumpeds_6properties.pdf", width=7, height=7)
...
theme(axis.title.x = element_blank(),axis.text.x=element_text(angle=90, vjust=0.5, size=4)) +
xlab("groupsizes")
ggsave(plot=p1, file="./images/groupsizesnonmqtl_clumpeds_6properties.pdf", width=7, height=7)



controllist<-list()
mqtl_clumpedlist<-list()

for (i in 1:dim(t)[1]){
cat(i,"\n")
if(t[i,1]>5&t[i,2]>0){
f.all.subgroup<-f.all[which(f.all$groups_quantiles2==row.names(t)[i]&f.all$mqtl_clumped==FALSE&f.all$snpchr!="chrX"),]
id<-f.all.subgroup[sample(nrow(f.all.subgroup), size=t[i,2], replace=FALSE),"SNP"]
controllist[[i]]<-id
w1<-which(f.all$groups_quantiles2==row.names(t)[i]&f.all$mqtl_clumped==TRUE)
mqtl_clumpedlist[[i]]<-f.all$SNP[w1]
}}

save(mqtl_clumpedlist,controllist,file=paste("../results/enrichments/controlslist6prop",no,".rdata",sep=""))
...
m<-match(f.all$groups_quantiles2,row.names(t))
t2<-t[m,]
f.all<-data.frame(f.all,Ncontrols_6prop_cis=t2[,1], Nmqtl_clumped_6prop_cis=t2[,2])
t<-data.frame(t[t[,2]>0,])
dim(t)
...
if(t[,2]>t[,1]) {id<-f.all.subgroup[sample(nrow(f.all.subgroup), size=t[i,2], replace=FALSE),"SNP"]}
if(t[,1]>t[,2]) {id<-f.all.subgroup[sample(nrow(f.all.subgroup), size=t[i,1], replace=FALSE),"SNP"]}
controllist[[i]]<-id
w1<-which(f.all$groups_quantiles2==row.names(t)[i]&f.all$snp_cis=="TRUE")
mqtl_clumpedlist[[i]]<-f.all$SNP[w1]
}}

save(mqtl_clumpedlist,controllist,file=paste("../results/enrichments/controlslist6prop_cis",no,".rdata",sep=""))
...
m<-match(f.all$groups_quantiles2,row.names(t))
t2<-t[m,]
f.all<-data.frame(f.all,Ncontrols_6prop_trans=t2[,1], Nmqtl_clumped_6prop_trans=t2[,2])
t<-data.frame(t[t[,2]>0,])
dim(t)
...
save(mqtl_clumpedlist,controllist,file=paste("../results/enrichments/controlslist6prop_trans",no,".rdata",sep=""))
...
m<-match(f.all$groups_quantiles2,row.names(t))
t2<-t[m,]
f.all<-data.frame(f.all,Ncontrols_6prop_amb=t2[,1], Nmqtl_clumped_6prop_amb=t2[,2])
t<-data.frame(t[t[,2]>0,])
dim(t)
...
save(mqtl_clumpedlist,controllist,file=paste("../results/enrichments/controlslist6prop_ambivalent",no,".rdata",sep=""))
